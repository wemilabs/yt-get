  ...
  try {
    ...
    if (!rateLimit.allowed) {
      const hoursRemaining = Math.ceil(
        (rateLimit.resetTime - Date.now()) / (1000 * 60 * 60)
      );

      return NextResponse.json(
        {
          error: "Rate limit exceeded",
          message: `You've reached the limit of 5 videos per 5 hours. Please try again in ${hoursRemaining} hour${
            hoursRemaining !== 1 ? "s" : ""
...
function processYtdlpResponse(info: Record<string, unknown>) {
  ...
  const typedInfo = info as YtdlpInfo;
  ...
  for (const format of videoWithAudio.sort(
    (a: Format, b: Format) =>
  ...
  for (const format of audioOnly.sort(
    (a: Format, b: Format) =>
      (b.abr || 0) - (a.abr || 0)
  )) {
